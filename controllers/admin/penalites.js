...
exports.postPenalite = async (req, res, next) => {
    try { 
        ...
        console.log(penaliteObj.montant)

        const user = await User.findOne({'numero':req.body.penalite.numero})
...
exports.deletePenalite = async (req, res, next) => {
    try{
        await Penalite.findOneAndUpdate({"user_id": req.params.user_id}, {"etat" : "paye"})
